...
def differential_entropy_timeresolved(power_seg, window=10):
    """
    V9.1 FIX: Time-resolved Differential Entropy computed from *local variance* (not local mean).
    Uses Var(x)=E[x^2]-(E[x])^2 for numerical stability.
    """
    ...
    x = power_seg.astype(np.float64)
    mean  = uniform_filter1d(x,   size=window, axis=1, mode='nearest')
    mean2 = uniform_filter1d(x*x, size=window, axis=1, mode='nearest')
...
def extract_subject_features(subj_dict, apply_ea=True, erp_template=None, return_eeg_proc=False):
    """
    ...
    if return_eeg_proc:
        return X, y, t, zmu, zsig, eeg_proc
...
# ── V9.1: Fold-wise ERP template without leakage (fast) ─────────────────────
def _compute_fold_erp_template_from_cache(cache_list, exclude_index,
                                         erp_start=ERP_WIN_START, erp_end=ERP_WIN_END):
    """Compute grand ERP (emo-neu) template excluding one subject (LOSO-safe).
    Uses cached per-subject class-mean ERPs to avoid recomputation.
    """
    emo_list, neu_list = [], []
    for j, c in enumerate(cache_list):
        if j == exclude_index:
            continue
        if c.get('erp_emo') is not None:
            emo_list.append(c['erp_emo'])
        if c.get('erp_neu') is not None:
...
def _inject_erp_features_into_X(X_in, eeg_proc, erp_template, base_dim, n_tp=N_TP):
    """Overwrite the ERP block (48 dims) in X_in with fold-specific ERP features.
    Assumes X layout: [base | ERP(48) | ITPC(16)].
    """
    erp_feat = apply_erp_template(eeg_proc, erp_template)  # (n_trials,48)
    erp_tiled = np.repeat(erp_feat, n_tp, axis=0)
    X_out = X_in.copy()
    X_out[:, base_dim:base_dim + (N_CH*3)] = erp_tiled
    return X_out
...
def run_loso(train_subjects, tune_sigma=True, verbose=True):
    ...
    for val_i in range(n):
        ...
        # V9.1: fold-wise ERP template (exclude validation subject to avoid leakage)
        erp_template_fold = _compute_fold_erp_template_from_cache(cache, val_i)
        # base feature dim where ERP block starts
        if val_i == 0:
            _BASE_DIM = val['X'].shape[1] - (N_CH*3) - N_CH

        tr  = [cache[i] for i in range(n) if i != val_i]

        X_tr       = np.vstack([_inject_erp_features_into_X(c['X'], c['eeg_proc'], erp_template_fold, _BASE_DIM) for c in tr])
        ...
        X_val = _inject_erp_features_into_X(val['X'], val['eeg_proc'], erp_template_fold, _BASE_DIM)
        X_v_f = np.hstack([X_val, csp_v])

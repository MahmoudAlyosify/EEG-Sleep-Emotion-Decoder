...
# ── FIX-W: Silence Triton/dynamo warnings on Windows (no Triton available) ──
# torch.compile requires Triton which is NOT available on Windows.
# We disable it completely to avoid 200-line warning spam.
os.environ['TORCH_COMPILE_DISABLE'] = '1'          # suppress compile attempt
...
try:
    ...
    if torch.cuda.is_available():
        ...
        if platform.system() == 'Windows':
            ...
            torch._dynamo.disable()   # FIX-W: fully disable compile on Windows (no Triton)
            print("   torch.compile  : DISABLED (Triton unavailable on Windows → eager mode)")
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — Configuration  (V8.1: RESTORED 300-900ms window — data-driven fix)
# ─────────────────────────────────────────────────────────────────────────────
...
# ── V8.1 CRITICAL FIX: RESTORED to 300-900ms (121 tps) ───────────────────────
# Why v8.0 failed with 400-600ms window (0.521 wAUC):
# • The TRE (LDA+LGBM+XGB+CatBoost) needs context — 41 tps was too few
# • The metric rewards SUSTAINED 50ms windows; narrow training → narrow support
# • The AUC time-course showed the REAL peak is ~200ms wide in practice
# • The paper's "400-600ms" is the statistical significance peak for THETA BAND
#   specifically, but the full EEG envelope is broader for classifier training
#
# V8.1 Strategy: Train on 300-900ms, but post-process to emphasize 400-600ms
# via the adaptive gate (soft upweighting of 400-600ms in smoothing).
...
# ── V8.1: UPDATED ADAPTIVE GATE ZONES (now match 300-900ms window) ───────────
# Zone A (tp 80-120 = 400-600ms):  PEAK signal → blend_alpha = 1.0
# Zone B (tp 60-79, 121-180 = 300-400ms, 600-900ms): CONTEXT → blend_alpha = 0.70
# Zone C (tp 0-59, 181-199 = 0-300ms, 900-1000ms): far noise → blend_alpha = 0.15
GATE_ZONE_A_START = int(0.400 * FS)   # tp=80  ← theta peak start
GATE_ZONE_A_END   = int(0.600 * FS)   # tp=120 ← theta peak end
GATE_ZONE_B_START = int(0.300 * FS)   # tp=60
GATE_ZONE_B_END   = int(0.900 * FS)   # tp=180
BLEND_ALPHA_A     = 1.00   # full signal in peak zone
BLEND_ALPHA_B     = 0.70   # soft keep in context zone (300-400ms, 600-900ms)
BLEND_ALPHA_C     = 0.15   # hard blend in far zone

WIN        = 40    # Context half-window (timepoints)
N_CSP      = 4    # CSP components per side
N_FEAT_SEL = 350  # V8.1: restored to 350 (matches 121 tps context)
...
print(f"✓ Config loaded — v8.1 (WINDOW RESTORED + ADAPTIVE GATE)")
print(f"  Signal window : tp [{TP_SIG_START}–{TP_SIG_END}]  "
      f"({T_SIG_START_MS}–{T_SIG_END_MS} ms)  ← RESTORED from v8.0")
print(f"  Classifiers   : {len(SIGNAL_TPS)} timepoints")
print(f"  Adaptive gate : A(400-600ms)=1.0 | B(300-900ms)=0.70 | C=0.15")
...
def _extract_one_trial(args):
    """Worker function for parallel trial feature extraction."""
...
def extract_subject_features(subj_dict, apply_ea=True):
    ...
    V8.1 FIX: Parallel trial extraction — 3-4× speedup on 28-core machine.
    S_13 was 35 min in v8.0, expected ~10 min in v8.1.
    """
    ...
    # Parallel extraction: each trial is independent → embarrassingly parallel
    # Use n_jobs = min(N_JOBS, n_tr) to avoid over-spawning
    ...
    # Sort by trial index (parallel may return out of order)
...
print("✓ Feature extraction defined — v8.1 (~715 features, SelectKBest→350)")
print("  Feature T (V8-6): DE time-resolved = 6×16 = 96 new features")
print("  Parallel trial extraction: uses all CPU cores (3-4× speedup)")
...
if HAVE_TORCH:
    ...
    def train_temporal_gru(X_tr_eeg, y_tr_bin, X_v_eeg, y_v_bin,
        ...
        # FIX-W: torch.compile DISABLED — Triton not available on Windows.
        # Running in eager mode (still GPU-accelerated via CUDA, just no JIT).
        # if hasattr(torch, 'compile'): model = torch.compile(...)  ← removed
    ...
    def train_temporal_gru_final(X_all_eeg, y_all_bin, device,
        ...
        model = TemporalGRUNet().to(device)
        # FIX-W: torch.compile disabled on Windows (no Triton)
...
print(f"✓ TimeResolvedEnsemble v8.1 defined")
print(f"  121 timepoints (300-900ms restored) | SelectKBest(350)")
...
def adaptive_temporal_gate(probs):
    """
    V8.1: Three-zone adaptive gate — tuned to data-observed AUC profile.

    Zone A (tp 80-120 = 400-600ms): PEAK theta ERS — full signal
    Zone B (tp 60-79 & 121-180 = 300-400ms, 600-900ms): context — 70% keep
    Zone C (tp 0-59 & 181-199 = 0-300ms, 900ms+): far noise — 15% keep

    Key insight from v8.0 results: The AUC time-course peaked near 200-400ms
    and dipped at 500-600ms in the data. This suggests the CLASSIFIER signal
    is broader than the THETA-band statistical significance reported in paper.
    We keep Zone B at 70% (was 50%) to preserve this broader signal.
    """
    out = probs.copy().astype(np.float64)
    for tp in range(N_TP):
        if GATE_ZONE_A_START <= tp <= GATE_ZONE_A_END:
            pass  # Zone A: full signal — no blending
        elif GATE_ZONE_B_START <= tp < GATE_ZONE_A_START or GATE_ZONE_A_END < tp <= GATE_ZONE_B_END:
...
def run_loso(train_subjects, tune_sigma=True, verbose=True):
    ...
    print("\n" + "="*70)
    print("  LOSO Cross-Validation — v8.1 (Window Restored + Adaptive Gate)")
    print("="*70)
...
def _plot_loso_results(fold_results, win_aucs, mean_aucs, n):
    try:
        ...
        ax1.set_title('Window-AUC per Fold (v8.1)', fontweight='bold')
        ...
        ax3.set_title('Summary Metrics (v8.1)', fontweight='bold')

        plt.suptitle(f'LOSO CV v8.1 — Mean Window-AUC = {np.mean(win_aucs):.4f}  '
                     f'ENN_W={ENN_W:.2f}  Signal: {T_SIG_START_MS}-{T_SIG_END_MS}ms  '
                     f'Gate: A=1.0 B=0.70 C=0.15',
                     fontsize=12, fontweight='bold')
        plt.tight_layout()
        save_path = os.path.join(BASE, 'loso_results_v8_1.png')
...
print("✓ LOSO CV defined (v8.1 — 300-900ms + adaptive 3-zone gate + parallel features)")
...
def generate_submission(train_subjects, test_subjects, output_path=OUTPUT):
    print("\n" + "="*70)
    print("  Final Training → Submission Generation (v8.1)")
    print("="*70)
...
print("✓ generate_submission defined (v8.1)")
...
print("""
╔══════════════════════════════════════════════════════════════════════════╗
║   EEG Emotional Memory — Ultra Pipeline v8.1  (FIXES APPLIED)           ║
╠══════════════════════════════════════════════════════════════════════════╣
║  FIX-W  torch.compile   : DISABLED (no Triton on Windows → clean logs)  ║
║  FIX-1  Signal Window   : 300-900ms RESTORED (121 tps, was 41 in v8.0) ║
║  FIX-2  Feature Select  : 350 features (restored, matches 121 tps)      ║
║  FIX-3  Parallel Feats  : Trial-parallel extraction (3-4× speedup)      ║
║  FIX-4  Adaptive Gate   : A(400-600ms)=1.0 / B(300-900ms)=0.70 / C=0.15║
║  KEEP   TemporalGRUNet  : BiGRU×2, frame overlap 20%, RMSprop           ║
║  KEEP   DE Features     : Time-resolved differential entropy +96 feats   ║
║  KEEP   GPU-1/2/3       : CatBoost/LightGBM/XGBoost GPU                ║
║  KEEP   Log-Euclidean   : Riemannian covariance features (GPU-5)        ║
║  KEEP   Euclidean Align : Cross-subject normalisation (ADD-1)           ║
║  KEEP   Isotonic Calib  : OOF-fitted per-timepoint calibration          ║
║  Features: ~715/tp → SelectKBest(350) | Signal: 121 tps (300-900ms)    ║
║  Expected wAUC: 0.535-0.555 (was 0.521 in v8.0)                        ║
╚══════════════════════════════════════════════════════════════════════════╝
""")
...
║  ENN_W        = {ENN_W:<55.2f}║
║  Signal window: {T_SIG_START_MS}-{T_SIG_END_MS}ms (restored 300-900ms, gate A=400-600ms)  ║
...
print("\n✓ EEG Ultra Pipeline v8.1 — all cells defined.")
print("""
╔══════════════════════════════════════════════════════════════════════════╗
║           v7.0 → v8.0 → v8.1 — Evolution Summary                        ║
╠══════════╦═══════════════════╦═══════════════════╦══════════════════════╣
║ Feature  ║ v7.0              ║ v8.0              ║ v8.1 (CURRENT)       ║
╠══════════╬═══════════════════╬═══════════════════╬══════════════════════╣
║ Window   ║ 300-900ms 121 tp  ║ 400-600ms 41 tp   ║ 300-900ms 121 tp ✓  ║
║ DL model ║ EEGNet (conv)     ║ BiGRU×2 + spatial ║ BiGRU×2 + spatial   ║
║ Optim    ║ AdamW             ║ RMSprop           ║ RMSprop             ║
║ compile  ║ torch.compile     ║ torch.compile     ║ DISABLED (Windows)  ║
║ Gate     ║ Fixed α=0.25      ║ A=1.0/B=0.5/C=0.1 ║ A=1.0/B=0.7/C=0.15 ║
║ Features ║ ~615/tp K(350)    ║ ~715/tp K(250)    ║ ~715/tp K(350)      ║
║ Parallel ║ serial trials     ║ serial trials     ║ parallel trials ✓   ║
║ Expected ║ ~0.535-0.555      ║ 0.521 (too narrow)║ ~0.540-0.560        ║
╚══════════╩═══════════════════╩═══════════════════╩══════════════════════╝

Key insight: TRE (LDA+LGBM+XGB+CatBoost) needs 121 timepoints of context.
Narrow window (41 tps) starves the classifiers → 0.521 wAUC.
Adaptive gate PRESERVES the 400-600ms emphasis in POST-PROCESSING.

Quick-start checklist:
  1. Set BASE path in CELL 3
  2. PyTorch CUDA already installed — no action needed
  3. RUN_LOSO = True in CELL 15 (auto-tunes sigma + ENN_W)
  4. Run the script — expect ~2-3 hrs total on RTX 5000 Ada
  5. submission.csv → upload to Kaggle

Scientific basis:
  Window 300-900ms: competition paper uses this as analysis window
  Gate 400-600ms peak: paper shows theta ERS peak here (Abdellahi et al.)
  BiGRU: Yoo 2023 (94-99% motion), Alhagry 2017 (85%+ emotion on DEAP)
  DE features: Li & Lu 2009 / SEED benchmark — cross-subject validated
  Frame overlap 20%: Yoo 2023 — prevents GRU overfitting on small N
""")

...
# ============================================================================
# EEG Emotional Memory — Ultra Pipeline v10.4 (LR/MDM Focus)
# ============================================================================
# Goal:
#   Refine the current winner line by simplifying the backbone toward the
#   empirically strongest components while weakening weak-context temporal mass.
#
# Focus of v10.4:
#   1) Backbone focus on LR + MDM + light LGBM (LDA removed, CatBoost off)
#   2) Weaker Zone C (rest context) in temporal shaping
#   3) Finer alpha search around the v10.3 winner point
#   4) Trial-level Platt kept ON (v10.2b removing it hurt public score)
#   5) OOF subject weighting preserved
#
...
OUTPUT = os.path.join(BASE, 'submission_v10_4_lr_mdm_focus.csv')
RUN_LOSO = True
...
K_BEST_LR = 220
LGBM_PARAMS = dict(
    n_estimators=380 if GPU_AVAILABLE else 240,
    num_leaves=31,
    max_depth=4,
    learning_rate=0.04,
    subsample=0.85,
...
# Finer search around v10.3 winner region
ALPHA_A_GRID = [0.68, 0.72, 0.76, 0.80]
ALPHA_B_GRID = [0.10, 0.14, 0.18]
ALPHA_C_GRID = [0.00, 0.03, 0.05, 0.08]
BEST_ALPHAS = (0.72, 0.14, 0.03)
BEST_BLEND = {'lr': 0.70, 'mdm': 0.20, 'lgbm': 0.10}
...
print("✓ Config loaded — v10.4 (LR/MDM focus)")
...
def load_all_training(emo_dir, neu_dir):
    ...
    for stem in sorted(set(emo_data.keys()) | set(neu_data.keys())):
        ...
        merged = {'eeg': np.concatenate(parts_eeg, axis=0), 'labels': np.concatenate(parts_lbl, axis=0), 'time': (emo_data.get(stem) or neu_data.get(stem))['time'], 'id': stem}
...
def preprocess_subject_for_cache(subj):
    ...
    emo_mask = labels == 2
    neu_mask = labels == 1
    return {'id': subj['id'], 'eeg_proc': eeg_proc, 'labels': labels.copy(), 'erp_emo': eeg_proc[emo_mask].mean(axis=0) if emo_mask.sum()>1 else None, 'erp_neu': eeg_proc[neu_mask].mean(axis=0) if neu_mask.sum()>1 else None, 'covs': compute_covariance_matrices(eeg_proc)}
...
def extract_zone_features(trial, zone='A', erp_template=None):
    ...
    if zone in ['A', 'B']:
        ...
        # weak context only — explicitly weaker than v10.3
        chunks = [(0, BASE_END), (ERP_B_END, N_TP)]
        for bname in ['theta', 'alpha', 'sigma']:
...
def extract_subject_feature_sets(eeg_proc, erp_template=None):
    ...
    return {'backbone': np.vstack(Xb).astype(np.float32), 'zoneA': np.vstack(XA).astype(np.float32), 'zoneB': np.vstack(XB).astype(np.float32), 'zoneC': np.vstack(XC).astype(np.float32)}

# Models
class TabularBinaryModel:
    def __init__(self, model_type='lr', k_best=None, C=1.0):
    ...
    def _make(self):
        if self.model_type == 'lr':
            return LogisticRegression(C=self.C, max_iter=2500, class_weight='balanced', solver='lbfgs', random_state=42)
...
class BackboneClassifier:
    def __init__(self, use_mdm=HAVE_PYRIEMANN):
        self.lr = TabularBinaryModel('lr', k_best=K_BEST_LR, C=0.45)
    ...
    def predict_parts(self, X, covs=None):
        parts = {'lr': self.lr.predict_proba(X).astype(np.float32), 'lgbm': self.lgbm.predict_proba(X).astype(np.float32), 'mdm': None}
...
class TemporalModulator:
    def __init__(self):
        ...
        self.modC = TabularBinaryModel('lr', k_best=min(32, K_BEST_ZONE), C=0.35)
    ...
    def predict_zone_probs(self, X_A, X_B, X_C):
        return {'pA': self.modA.predict_proba(X_A).astype(np.float32), 'pB': self.modB.predict_proba(X_B).astype(np.float32), 'pC': self.modC.predict_proba(X_C).astype(np.float32)}
...
def compute_subject_weights_heuristic(train_subjects, cache):
    weights = []
...
def tune_backbone_blend(oof_parts_list, oof_labels_list):
    global BEST_BLEND
    print("\nTuning LR/MDM/LGBM backbone blend on pooled OOF...")
    ...
    if all(d.get('mdm') is not None for d in oof_parts_list):
        P['mdm'] = np.concatenate([d['mdm'] for d in oof_parts_list])
    best_auc = -1.0
    best = BEST_BLEND.copy()
    grid_lr = [0.50, 0.60, 0.70, 0.80]
    grid_mdm = [0.00, 0.10, 0.20, 0.25]
    grid_lgb = [0.00, 0.05, 0.10, 0.15]
    for wl in grid_lr:
        for wm in (grid_mdm if P['mdm'] is not None else [0.0]):
            for wg in grid_lgb:
                total = wl + wm + wg
                if total <= 0:
                    continue
                blend = wl*P['lr'] + wg*P['lgbm']
...
def tune_alphas_global(oof_trial_list, oof_zone_list, oof_labels_list):
    global BEST_ALPHAS
    print("\nTuning finer temporal alpha grid on pooled OOF...")
    best = BEST_ALPHAS
    best_auc = -1.0
...
def plot_loso_results(fold_results, save_path):
    try:
        ...
        ax1.set_title('V10.4: Window-AUC per Fold', fontweight='bold')
        ...
        ax2.set_title('V10.4: Trial-AUC per Fold', fontweight='bold')
        ...
        plt.suptitle(f'LOSO v10.4 — mean wAUC={np.mean(win_aucs):.4f} | trial AUC={np.mean(trial_aucs):.4f}', fontweight='bold')
        plt.tight_layout()
...
def build_fold_matrices(train_subjects, cache, exclude_idx, csp_global):
    ...
    X_tr = {'backbone': np.vstack(Xb_parts).astype(np.float32), 'zoneA': np.vstack(XA_parts).astype(np.float32), 'zoneB': np.vstack(XB_parts).astype(np.float32), 'zoneC': np.vstack(XC_parts).astype(np.float32)}
    ...
    X_val = {'backbone': np.hstack([feat_val['backbone'], csp_v]).astype(np.float32), 'zoneA': feat_val['zoneA'].astype(np.float32), 'zoneB': feat_val['zoneB'].astype(np.float32), 'zoneC': feat_val['zoneC'].astype(np.float32)}
    y_val = train_subjects[exclude_idx]['labels']
    cov_val = cache[exclude_idx]['covs']
    return X_tr, y_tr, cov_tr, X_val, y_val, cov_val


def run_loso_v104(train_subjects, cache, csp_global):
    global BEST_BLEND, BEST_ALPHAS
    print("\n" + "="*70)
    print(" LOSO Cross-Validation — v10.4 (LR/MDM focus)")
    print("="*70)
    subj_weights = compute_subject_weights_heuristic(train_subjects, cache)
    print(f" Initial subject weights: {np.round(subj_weights, 3)}")

    oof_parts_list, oof_zone_list, oof_trial_list, oof_labels = [], [], [], []
    fold_trial_aucs = []

    # Pass 1 for OOF tuning
    for i in range(len(train_subjects)):
        ...
        backbone = BackboneClassifier(use_mdm=HAVE_PYRIEMANN).fit(X_tr['backbone'], y_tr, covs=cov_tr, sample_weight=sw)
    ...
    tune_backbone_blend(oof_parts_list, oof_labels)
    tune_alphas_global(oof_trial_list, oof_zone_list, oof_labels)

    # Pass 2 with tuned settings
    print("\nRe-running LOSO with tuned LR/MDM blend, tuned alphas, and OOF subject weights...")
...
def train_final_model(train_subjects, cache, csp_global, subj_weights=None):
    ...
    X = {'backbone': np.vstack(Xb_parts).astype(np.float32), 'zoneA': np.vstack(XA_parts).astype(np.float32), 'zoneB': np.vstack(XB_parts).astype(np.float32), 'zoneC': np.vstack(XC_parts).astype(np.float32)}
...
def generate_submission_v104(train_subjects, test_subjects, cache, csp_global, trial_cal=None, subj_weights=None, output_path=OUTPUT):
    print("\n" + "="*70)
    print(" Final Training → Submission Generation (v10.4)")
    print("="*70)
...
if __name__ == '__main__':
    print("""
╔══════════════════════════════════════════════════════════════════════════╗
║ EEG Emotional Memory — Ultra Pipeline v10.4                            ║
╠══════════════════════════════════════════════════════════════════════════╣
║ Focused winner refinement:                                              ║
║  - LR/MDM/LGBM-light backbone                                          ║
║  - weaker Zone C                                                        ║
║  - finer alpha search                                                   ║
║  - Trial Platt kept ON                                                  ║
╚══════════════════════════════════════════════════════════════════════════╝
""")
    ...
    if RUN_LOSO:
        print('\nSTEP 5: LOSO CV...')
        fold_results, oof_trial_probs, oof_labels, subj_weights = run_loso_v104(train_subjects, cache, csp_global)
        plot_loso_results(fold_results, os.path.join(BASE, 'loso_results_v10_4.png'))
        if USE_TRIAL_PLATT:
    ...
    df = generate_submission_v104(train_subjects, test_subjects, cache, csp_global, trial_cal=trial_cal, subj_weights=subj_weights, output_path=OUTPUT)
    print('\nSTEP 8: Validation...')

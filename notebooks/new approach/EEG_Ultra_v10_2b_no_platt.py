...
# ============================================================================
# EEG Emotional Memory — Ultra Pipeline v10.2b (NO TRIAL PLATT)
# ============================================================================
# Trial-level backbone + 3-zone temporal modulator.
# Difference from v10.2: removes trial-level Platt calibration to preserve raw ranking.
...
OUTPUT = os.path.join(BASE, 'submission_v10_2b_no_platt.csv')
RUN_LOSO = True
...
print("✓ Config loaded — v10.2b (NO TRIAL PLATT)")
...
class BackboneClassifier:
    ...
    def predict(self, X, covs=None):
        ...
        return {'p_trial': p_final.astype(np.float32)}
...
def plot_loso_results(fold_results, save_path):
    try:
        ...
        ax1.set_title('V10.2b: Window-AUC per Fold', fontweight='bold')
        ...
        ax2.set_title('V10.2b: Trial-AUC per Fold', fontweight='bold')
        ...
        plt.suptitle(f'LOSO v10.2b — mean wAUC={np.mean(win_aucs):.4f} | trial AUC={np.mean(trial_aucs):.4f}', fontweight='bold')
        plt.tight_layout()
...
def run_loso_v102b(train_subjects, cache, csp_global):
    global BEST_ALPHAS
    print("\n" + "="*70)
    print(" LOSO Cross-Validation — v10.2b (NO TRIAL PLATT)")
    print("="*70)
    ...
    for i in range(len(train_subjects)):
        ...
        gc.collect()
    BEST_ALPHAS, _ = tune_alphas_global(oof_trial_probs, oof_zone_probs, oof_labels)
    ...
    for p_trial, z, lbl in zip(oof_trial_probs, oof_zone_probs, oof_labels):
        ...
        try:
            ...
            trial_auc = 0.5
        fold_results.append({**window_auc_score(np.clip(probs,0.01,0.99), yb), 'trial_auc': float(trial_auc)})
    win_aucs = [r['window_auc'] for r in fold_results]
    trial_aucs = [r['trial_auc'] for r in fold_results]
    mean_aucs = [r['mean_auc'] for r in fold_results]
    print(f"\n ╔═══════════════════════════════════════════════════════════╗")
    print(f" ║ V10.2b — CORE + TEMPORAL MODULATOR (NO PLATT)           ║")
...
def generate_submission_v102b(train_subjects, test_subjects, cache, csp_global, output_path=OUTPUT):
    print("\n" + "="*70)
    print(" Final Training → Submission Generation (v10.2b, no trial platt)")
    print("="*70)
...
if __name__ == '__main__':
    print("""
╔══════════════════════════════════════════════════════════════════════════╗
║ EEG Emotional Memory — Ultra Pipeline v10.2b                           ║
╠══════════════════════════════════════════════════════════════════════════╣
║ Trial-level backbone + 3-zone temporal modulator                       ║
║ NO trial-level Platt calibration                                       ║
║ Same paths as your current environment                                 ║
╚══════════════════════════════════════════════════════════════════════════╝
""")
    ...
    if RUN_LOSO:
        print('\nSTEP 5: LOSO CV...')
        fold_results = run_loso_v102b(train_subjects, cache, csp_global)
        plot_loso_results(fold_results, os.path.join(BASE, 'loso_results_v10_2b.png'))
        print('\nSTEP 6: Trial-level Platt calibration... SKIPPED in v10.2b')
    ...
    df = generate_submission_v102b(train_subjects, test_subjects, cache, csp_global, OUTPUT)
    print('\nSTEP 8: Validation...')

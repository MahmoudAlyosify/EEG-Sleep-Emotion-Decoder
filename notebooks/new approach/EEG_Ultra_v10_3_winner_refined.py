...
# ============================================================================
# EEG Emotional Memory — Ultra Pipeline v10.3 (WINNER REFINED)
# ============================================================================
# Accuracy-first refinement of the current winner line:
#   TRIAL-LEVEL BACKBONE + 3-ZONE TEMPORAL MODULATOR + TRIAL PLATT
#
# New in v10.3:
#   1) OOF-tuned BACKBONE blend weights (LDA / LGBM / LR / MDM)
#   2) Narrower, accuracy-focused temporal alpha search around the winning zone-A peak
#   3) Subject OOF-derived sample weighting for final training (better than heuristic proxy)
#   4) Stronger feature set in backbone and zone heads while staying trial-level
#   5) Optional CatBoost GPU expert in backbone (off by default; easy toggle)
#
# Proven path preserved from v10.2:
#   - Trial-level backbone (generalizes better than per-timepoint TRE/GRU)
#   - 3-zone temporal shaping (beats flat tiling on public leaderboard)
#   - Trial-level Platt kept ON by default (v10.2b showed removing it hurt public score)
# ============================================================================
...
for pkg in ['lightgbm', 'tqdm', 'pyriemann', 'catboost']:
    try:
...
N_TP = 200
N_CH = 16
ERP_A_START = int(0.100 * FS)
ERP_A_END   = int(0.350 * FS)
ERP_B_START = int(0.350 * FS)
ERP_B_END   = int(0.700 * FS)
BASE_START  = 0
BASE_END    = int(0.100 * FS)
...
CHANNELS = ['c3','c4','o1','o2','cp3','f3','f4','cp4','c5','cz','c6','cp5','p7','pz','p8','cp6']
CH = {c: i for i, c in enumerate(CHANNELS)}
ASYM_PAIRS = [('c3','c4'), ('f3','f4'), ('cp3','cp4'), ('p7','p8')]
ASYM_BANDS = ['theta', 'alpha', 'beta']
CONN_PAIRS = [('f3','pz'), ('f4','pz'), ('f3','cz'), ('f4','cz'), ('c3','c4'), ('cp3','cp4'), ('f3','f4'), ('cz','pz'), ('f3','cp4'), ('f4','cp3')]
...
OUTPUT = os.path.join(BASE, 'submission_v10_3_winner_refined.csv')
RUN_LOSO = True
USE_CATBOOST = False  # accuracy experiment toggle; OFF by default to reduce variance
USE_TRIAL_PLATT = True

# Backbone settings
K_BEST_LDA = 170
K_BEST_ZONE = 96
LGBM_PARAMS = dict(
    n_estimators=550 if GPU_AVAILABLE else 300,
    num_leaves=31,
    max_depth=5,
    learning_rate=0.035,
    subsample=0.85,
...
# Narrow search around observed winner region from v10.2
ALPHA_A_GRID = [0.60, 0.70, 0.80, 0.90]
ALPHA_B_GRID = [0.00, 0.05, 0.10, 0.15]
ALPHA_C_GRID = [0.00, 0.03, 0.05, 0.08, 0.10]
BEST_ALPHAS = (0.70, 0.10, 0.05)
BEST_BLEND = {'lda': 0.25, 'lgbm': 0.45, 'lr': 0.15, 'mdm': 0.15, 'cat': 0.0}

print(f"GPU_AVAILABLE={GPU_AVAILABLE} HAVE_TORCH={HAVE_TORCH} TORCH_DEVICE={TORCH_DEVICE}")
print(f"PyRiemann={'YES' if HAVE_PYRIEMANN else 'NO'} | CatBoost={'YES' if HAVE_CATBOOST else 'NO'}")
print("✓ Config loaded — v10.3 (winner refined)")
...
def extract_backbone_features(trial, erp_template=None):
    ...
    # Spindle block
    feat.extend(np.log1p(np.mean(bp_power['sigma'][:, ERP_A_START:ERP_B_END], axis=1)).tolist())
    # Global context extras (creative but still trial-level)
    feat.extend(np.log1p(np.var(trial[:, :BASE_END], axis=1)).tolist())
    feat.extend(np.log1p(np.var(trial[:, ERP_B_END:], axis=1)).tolist())
...
def extract_zone_features(trial, zone='A', erp_template=None):
    ...
    if zone in ['A', 'B']:
        ...
        for bname in BAND_NAMES:
            vals = [np.mean(bp_power[bname][:, s:e], axis=1) for s,e in chunks]
            feat.extend(np.log1p(np.mean(vals, axis=0)).tolist())
...
def extract_subject_feature_sets(eeg_proc, erp_template=None):
    ...
    for i in range(eeg_proc.shape[0]):
        tr = eeg_proc[i]
        Xb.append(extract_backbone_features(tr, erp_template))
        XA.append(extract_zone_features(tr, 'A', erp_template))
        XB.append(extract_zone_features(tr, 'B', erp_template))
        XC.append(extract_zone_features(tr, 'C', erp_template))
    return {
        'backbone': np.vstack(Xb).astype(np.float32),
        'zoneA': np.vstack(XA).astype(np.float32),
        'zoneB': np.vstack(XB).astype(np.float32),
        'zoneC': np.vstack(XC).astype(np.float32),
    }
...
class TabularBinaryModel:
    ...
    def _make(self):
        ...
        if self.model_type == 'cat':
            if not HAVE_CATBOOST:
                raise ValueError('CatBoost unavailable')
            params = dict(iterations=350 if GPU_AVAILABLE else 220,
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        try:
            if sample_weight is not None and self.model_type in ['lr', 'lgbm', 'cat']:
...
class BackboneClassifier:
    def __init__(self, use_mdm=HAVE_PYRIEMANN, use_cat=False):
        ...
        self.use_cat = use_cat and HAVE_CATBOOST
        self.cat = TabularBinaryModel('cat', k_best=None) if self.use_cat else None
        self.mdm = None
        self.weights = BEST_BLEND.copy()
    def fit(self, X, y, covs=None, sample_weight=None):
        ...
        if self.use_cat:
            self.cat.fit(X, yb, sample_weight=sample_weight)
        if self.use_mdm and covs is not None:
            self.mdm = MDM(metric='riemann')
            self.mdm.fit(covs, yb)
        return self
    def predict_parts(self, X, covs=None):
        parts = {
            'lda': self.lda.predict_proba(X).astype(np.float32),
            'lgbm': self.lgbm.predict_proba(X).astype(np.float32),
            'lr': self.lr.predict_proba(X).astype(np.float32),
            'cat': None,
            'mdm': None,
        }
        if self.use_cat:
            parts['cat'] = self.cat.predict_proba(X).astype(np.float32)
        if self.use_mdm and self.mdm is not None and covs is not None:
            ...
            parts['mdm'] = (p[:, idx1[0]] if len(idx1) > 0 else p[:, -1]).astype(np.float32)
        return parts
    def blend(self, parts, weights=None):
        if weights is None:
        ...
        for k, w in weights.items():
            ...
            if p is not None and w > 0:
                ...
                den = den + w
        if den <= 0:
            n = len(next(v for v in parts.values() if v is not None))
            return np.full(n, 0.5, dtype=np.float32)
        return (num / den).astype(np.float32)
    def predict(self, X, covs=None):
        parts = self.predict_parts(X, covs)
        return {'p_trial': self.blend(parts), **{f'p_{k}': v for k, v in parts.items()}}
...
class TemporalModulator:
    def __init__(self):
        ...
        self.modC = TabularBinaryModel('lr', k_best=min(48, K_BEST_ZONE), C=0.5)
...
# ─────────────────────────────────────────────────────────────────────────────
# Utilities: weights, tuning, metric, plots
# ─────────────────────────────────────────────────────────────────────────────
def compute_subject_reliability_weights_heuristic(train_subjects, cache):
    weights = []
    for s, c in zip(train_subjects, cache):
        ...
        try:
            ...
            auc = 0.5
        rel = 0.5 + max(0.0, auc - 0.5) * 2.0
        weights.append(float(np.clip(rel, 0.50, 1.00)))
    return np.array(weights, dtype=np.float32)


def expand_subject_weights(subject_weights, train_subjects, exclude_idx=None):
    out = []
...
def tune_backbone_blend(oof_parts_list, oof_labels_list, use_cat=False, use_mdm=False):
    global BEST_BLEND
    print("\nTuning backbone blend weights on pooled OOF...")
    y = np.concatenate([(lbl == 2).astype(int) for lbl in oof_labels_list])
    P = {
        'lda': np.concatenate([d['lda'] for d in oof_parts_list]),
        'lgbm': np.concatenate([d['lgbm'] for d in oof_parts_list]),
        'lr': np.concatenate([d['lr'] for d in oof_parts_list]),
        'mdm': None,
        'cat': None,
    }
    if use_mdm and all(d.get('mdm') is not None for d in oof_parts_list):
        P['mdm'] = np.concatenate([d['mdm'] for d in oof_parts_list])
    if use_cat and all(d.get('cat') is not None for d in oof_parts_list):
        P['cat'] = np.concatenate([d['cat'] for d in oof_parts_list])

    best_auc = -1.0
    best = BEST_BLEND.copy()
    grid = [0.0, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]
    for wl in grid:
        for wg in grid:
            for wr in grid:
                for wm in ([0.0, 0.10, 0.15, 0.20] if P['mdm'] is not None else [0.0]):
                    for wc in ([0.0, 0.05, 0.10, 0.15] if P['cat'] is not None else [0.0]):
                        total = wl + wg + wr + wm + wc
                        if total <= 0.0:
                            continue
                        blend = (wl*P['lda'] + wg*P['lgbm'] + wr*P['lr'])
                        if P['mdm'] is not None:
                            blend = blend + wm*P['mdm']
                        if P['cat'] is not None:
                            blend = blend + wc*P['cat']
                        blend = blend / total
                        try:
                        ...
                        if auc > best_auc:
                            best_auc = auc
                            best = {'lda': wl, 'lgbm': wg, 'lr': wr, 'mdm': wm, 'cat': wc}
    BEST_BLEND = best
    print(f" Best backbone blend: {BEST_BLEND} | pooled trial-AUC={best_auc:.4f}")
...
def plot_loso_results(fold_results, save_path):
    try:
        ...
        ax1.set_title('V10.3: Window-AUC per Fold', fontweight='bold')
        ...
        ax2.set_title('V10.3: Trial-AUC per Fold', fontweight='bold')
        ...
        plt.suptitle(f'LOSO v10.3 — mean wAUC={np.mean(win_aucs):.4f} | trial AUC={np.mean(trial_aucs):.4f}', fontweight='bold')
        plt.tight_layout()
...
def run_loso_v103(train_subjects, cache, csp_global):
    global BEST_BLEND, BEST_ALPHAS
    print("\n" + "="*70)
    print(" LOSO Cross-Validation — v10.3 (winner refined)")
    print("="*70)

    # First-pass subject weights (heuristic) to stabilize OOF estimation
    subj_weights = compute_subject_reliability_weights_heuristic(train_subjects, cache)
    print(f" Initial subject weights: {np.round(subj_weights, 3)}")

    oof_parts_list, oof_zone_list, oof_trial_list, oof_labels = [], [], [], []
    fold_trial_aucs = []

    # Pass 1: collect OOF parts for blend and alpha tuning
    for i in range(len(train_subjects)):
        ...
        backbone = BackboneClassifier(use_mdm=HAVE_PYRIEMANN, use_cat=USE_CATBOOST).fit(X_tr['backbone'], y_tr, covs=cov_tr, sample_weight=sw)
        ...
        try:
            tr_auc = roc_auc_score(yb, p_trial)
        except Exception:
            tr_auc = 0.5
        fold_trial_aucs.append(tr_auc)
        oof_parts_list.append(parts)
        oof_trial_list.append(p_trial.astype(np.float32))
        oof_zone_list.append({k: v.astype(np.float32) for k,v in zone_probs.items()})
        oof_labels.append(y_val.copy())
        print(f" Pass1 Fold {i+1:2d} | {train_subjects[i]['id'][:22]:22s} | trial_AUC={tr_auc:.4f} ({time.time()-t0:.0f}s)")
        gc.collect()

    # OOF-derived subject weights from pass1 trial AUC
    subj_weights = np.array([0.5 + max(0.0, auc - 0.5) * 3.0 for auc in fold_trial_aucs], dtype=np.float32)
    subj_weights = np.clip(subj_weights, 0.50, 1.15)
    print(f" OOF-derived subject weights: {np.round(subj_weights, 3)}")

    tune_backbone_blend(oof_parts_list, oof_labels, use_cat=USE_CATBOOST, use_mdm=HAVE_PYRIEMANN)
    tune_alphas_global(oof_trial_list, oof_zone_list, oof_labels)

    # Pass 2: evaluate with tuned blend + tuned alphas + OOF subject weights
    print("\nRe-running LOSO with tuned backbone blend, tuned alphas, and OOF subject weights...")
    fold_results, oof_trial_probs_final, oof_labels_final = [], [], []
    for i in range(len(train_subjects)):
        t0 = time.time()
        X_tr, y_tr, cov_tr, X_val, y_val, cov_val = build_fold_matrices(train_subjects, cache, i, csp_global)
        sw = expand_subject_weights(subj_weights, train_subjects, i)
        backbone = BackboneClassifier(use_mdm=HAVE_PYRIEMANN, use_cat=USE_CATBOOST)
        backbone.weights = BEST_BLEND.copy()
        ...
        if USE_TRIAL_PLATT:
            # no fold leakage: fit small calibrator on training-trial proxy from backbone OOF is ideal,
            # but to keep runtime reasonable, we defer calibration to pooled OOF after folds.
            pass
        yb = (y_val == 2).astype(int)
        try:
        ...
        oof_trial_probs_final.append(p_trial.astype(np.float32))
        oof_labels_final.append(y_val.copy())
    ...
    print(f" ║ Blend      : {BEST_BLEND} ║")
    print(f" ║ Alphas     : A={BEST_ALPHAS[0]:.2f} B={BEST_ALPHAS[1]:.2f} C={BEST_ALPHAS[2]:.2f}          ║")
    print(f" ╚═══════════════════════════════════════════════════════════╝")
    return fold_results, oof_trial_probs_final, oof_labels_final, subj_weights

# ─────────────────────────────────────────────────────────────────────────────
# Final training + submission
# ─────────────────────────────────────────────────────────────────────────────
def train_final_model(train_subjects, cache, csp_global, subj_weights=None):
    if subj_weights is None:
        subj_weights = compute_subject_reliability_weights_heuristic(train_subjects, cache)
    tmpl = compute_fold_erp_template(cache, exclude_idx=-1)
    Xb_parts, XA_parts, XB_parts, XC_parts, y_parts, cov_parts = [], [], [], [], [], []
    for i, (s, c) in enumerate(zip(train_subjects, cache)):
    ...
    backbone = BackboneClassifier(use_mdm=HAVE_PYRIEMANN, use_cat=USE_CATBOOST)
    backbone.weights = BEST_BLEND.copy()
    backbone.fit(X['backbone'], y, covs=covs, sample_weight=sw)
    mod = TemporalModulator().fit(X['zoneA'], X['zoneB'], X['zoneC'], y, sample_weight=sw)
    mod.alphas = BEST_ALPHAS
    return backbone, mod, tmpl


def generate_submission_v103(train_subjects, test_subjects, cache, csp_global, trial_cal=None, subj_weights=None, output_path=OUTPUT):
    print("\n" + "="*70)
    print(" Final Training → Submission Generation (v10.3)")
    print("="*70)
    backbone, mod, tmpl = train_final_model(train_subjects, cache, csp_global, subj_weights=subj_weights)
    rows = []
    for s in test_subjects:
        ...
        covs = compute_covariance_matrices(eeg_proc)
        p_trial = backbone.predict(Xb, covs=covs)['p_trial']
        if USE_TRIAL_PLATT and trial_cal is not None:
...
if __name__ == '__main__':
    print("""
╔══════════════════════════════════════════════════════════════════════════╗
║ EEG Emotional Memory — Ultra Pipeline v10.3                            ║
╠══════════════════════════════════════════════════════════════════════════╣
║ Winner refinement:                                                      ║
║  - Trial-level backbone (LDA+LGBM+LR+MDM)                             ║
║  - OOF-tuned backbone blend                                            ║
║  - 3-zone temporal modulator with narrowed alpha search               ║
║  - OOF subject weighting                                               ║
║  - Trial Platt kept ON by default                                      ║
╚══════════════════════════════════════════════════════════════════════════╝
""")
    ...
    if RUN_LOSO:
        print('\nSTEP 5: LOSO CV...')
        fold_results, oof_trial_probs, oof_labels, subj_weights = run_loso_v103(train_subjects, cache, csp_global)
        plot_loso_results(fold_results, os.path.join(BASE, 'loso_results_v10_3.png'))
        if USE_TRIAL_PLATT:
            print('\nSTEP 6: Trial-level Platt calibration...')
            trial_cal = fit_trial_platt(oof_trial_probs, oof_labels)
        else:
            print('\nSTEP 6: Trial-level Platt calibration... SKIPPED')
            trial_cal = None
    ...
    df = generate_submission_v103(train_subjects, test_subjects, cache, csp_global, trial_cal=trial_cal, subj_weights=subj_weights, output_path=OUTPUT)
